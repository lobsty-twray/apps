class LobstyBoard {
    ...
    async createTask(data) {
        const task = await this.apiCall(`/api/projects/${this.currentProject.id}/tasks`, {
            method: 'POST',
            body: JSON.stringify(data)
        });
    ...
    async moveTask(taskId, status, position = 0) {
        await this.apiCall(`/api/tasks/${taskId}/move`, {
            method: 'PATCH',
    ...
    async createLabel(data) {
        const label = await this.apiCall('/api/labels', {
            method: 'POST',
            body: JSON.stringify(data)
        });
            ...
            <div class="task-meta">
                ...
                <div class="task-labels">
                    ${labels.map((label, index) => `
                        <span class="label-tag" style="background-color: ${labelColors[index] || '#646cff'}">${this.escapeHtml(label)}</span>
                    `).join('')}
                </div>
            </div>
        `;
